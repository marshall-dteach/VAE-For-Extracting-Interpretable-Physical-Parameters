...
class DynamicConv2d(nn.Layer):
    ...
    def forward(self, input, weight, bias):
        ...
        image_patches = F.unfold(y, list(self.kernel_size), list(self.dilation), 0, list(self.stride)).transpose([0, 2, 1])
        y = image_patches.matmul(weight.reshape([-1, self.in_channels * self.kernel_size[0] * self.kernel_size[1], self.out_channels]))
        if bias is not None:
            y = y + bias.reshape([-1, 1, self.out_channels])

        return y.transpose([0, 2, 1]).reshape([-1, self.out_channels, output_size[0], output_size[1]])
...
class ConvPropagator(nn.Layer):
    def __init__(self, hidden_channels, linear_kernel_size, nonlin_kernel_size, data_channels, stride=1,
        ...
        if prop_layers > 0:
            self.conv_prop = nn.LayerList([DynamicConv2d(hidden_channels, hidden_channels, nonlin_kernel_size, stride,
                                            nonlin_padding, dilation, groups, boundary_cond)
        ...
        self.cutoff = paddle.create_parameter(shape=cutoff.shape,
                                              ...
                                              default_initializer=nn.initializer.Assign(cutoff))

    def _target_pad_2d(self, y, y0):
        y = paddle.concat([y0[:,:,:self.padding, self.padding:-self.padding], 
                           y, y0[:,:,-self.padding:,  self.padding:-self.padding]], axis=-2)
    ...
    def forward(self, y0, linear_weight, linear_bias, 
        ...
        for i in range(depth):
            ...
            noise = self.prop_noise * paddle.randn(y.shape) if (self.training and self.prop_noise > 0) else 0
            y = y + self.cutoff * paddle.tanh((dt * f(y_pad)) / self.cutoff) + noise

            y_list.append(y)

        return paddle.stack(y_list, axis=-3)
